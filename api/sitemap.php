...
$base = base_url();
$rows = db()
    ->query("SELECT id, created_at FROM resources WHERE status = 'approved' ORDER BY id")
...
<?php foreach ($rows as $row): ?>
  <url><loc><?= htmlspecialchars($base) ?>/documento/<?= (int) $row['id'] ?></loc><lastmod><?= $lastmod($row['created_at']) ?></lastmod></url>
<?php endforeach; ?>
</urlset>
